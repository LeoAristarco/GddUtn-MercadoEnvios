...
CREATE TABLE LOS_INSISTENTES.Visibilidades
(
	...
	Permite_Envios			BIT DEFAULT 0 NOT NULL
...
(
	...
	FOREIGN KEY (Cod_Visibilidad) REFERENCES LOS_INSISTENTES.Visibilidades(Cod_Visibilidad),
...
CREATE PROCEDURE LOS_INSISTENTES.sp_AgregarFuncionalidadRol(@rol nvarchar(255), @funcionalidad nvarchar(255))
AS
BEGIN
	INSERT INTO LOS_INSISTENTES.Funcionalidad_Rol (ID_Rol, ID_Funcionalidad)
		VALUES ((SELECT ID_Rol FROM LOS_INSISTENTES.Roles WHERE Nombre = @rol),
...
AS BEGIN
	...
	UPDATE LOS_INSISTENTES.Facturas
	SET Total_Facturacion = Total_Facturacion + (@itemMonto * @itemCantidad)
	WHERE Nro_Factura = @idFactura
END
GO
...
CREATE PROCEDURE LOS_INSISTENTES.sp_InsertarOferta 
	(@idComprador numeric (18,0), @codPublicacion numeric (18,0), 
	 @fechaOferta datetime, @montoOferta numeric (18,2))
AS BEGIN
		INSERT INTO LOS_INSISTENTES.Subastas
			(ID_Comprador,Cod_Publicacion, Fecha_Oferta, Monto_Oferta)
		VALUES 
			(@idComprador, @codPublicacion, @fechaOferta, @montoOferta)
END
GO
...
SELECT ID_Comprador, t.Cod_Publicacion, Descripcion, Fecha_Oferta AS Fecha_Operacion, Monto_Oferta, 'Oferta en subasta' AS Tipo
		FROM LOS_INSISTENTES.Subastas t, LOS_INSISTENTES.Publicaciones p
		WHERE t.Cod_Publicacion NOT IN (
			SELECT Cod_Publicacion FROM LOS_INSISTENTES.Compras c WHERE t.ID_Comprador = c.ID_Comprador
		) AND t.Cod_Publicacion = p.Cod_Publicacion
UNION
SELECT ID_Comprador, t.Cod_Publicacion, Descripcion, Fecha_Oferta AS Fecha_Operacion, Monto_Oferta, 'Subasta adjudicada' AS Tipo
		FROM LOS_INSISTENTES.Subastas t, LOS_INSISTENTES.Publicaciones p
		WHERE t.Cod_Publicacion IN (
			SELECT Cod_Publicacion FROM LOS_INSISTENTES.Compras c WHERE t.ID_Comprador = c.ID_Comprador
		) AND t.Cod_Publicacion = p.Cod_Publicacion
UNION
...
AS
	...
	SELECT * FROM (
		SELECT ROW_NUMBER() OVER(ORDER BY e.Descripcion, Cod_Publicacion DESC) AS Fila, Cod_Publicacion, p.Descripcion AS Descripcion,  r.Descripcion AS Rubro, t.Descripcion As Tipo, v.Descripcion AS Visibilidad, e.Descripcion AS Estado
	...
	ORDER BY Estado, Cod_Publicacion DESC
GO
...
AS
		...
		SET @SQLQuery = 'SELECT * FROM (
			SELECT ROW_NUMBER() OVER(ORDER BY v.Costo_Publicacion DESC) AS Fila, Cod_Publicacion, Costo_Publicacion, p.Descripcion AS Descripcion,  r.Descripcion AS Rubro, t.Descripcion As Tipo, e.Descripcion AS Estado, p.Precio AS montoCompra
			FROM LOS_INSISTENTES.Publicaciones p
			JOIN LOS_INSISTENTES.Tipos_Publicacion t on t.Cod_TipoPublicacion = p.Cod_TipoPublicacion
			JOIN LOS_INSISTENTES.Visibilidades v on v.Cod_Visibilidad = p.Cod_Visibilidad
			JOIN LOS_INSISTENTES.Estados_Publicacion e on e.Cod_EstadoPublicacion = p.Cod_EstadoPublicacion
			JOIN LOS_INSISTENTES.Rubros r on r.ID_Rubro = p.ID_Rubro
			WHERE ID_Vendedor != @ID_User AND p.Cod_EstadoPublicacion IN (1,2)'
		...
		SET @SQLQuery = @SQLQuery + ') 
				AS T WHERE Fila BETWEEN ((@pagina) + (19*(@pagina-1))) AND ((@pagina) + (19*(@pagina-1))) + 19
...
CREATE VIEW LOS_INSISTENTES.vw_ProductosNoVendidos AS
	SELECT	LOS_INSISTENTES.fc_nombre(ID_Vendedor)		AS Vendedor,
			...
			Visibilidades.Cod_Visibilidad				AS Cod_Visibilidad,
			...
			JOIN LOS_INSISTENTES.Visibilidades ON Publicaciones.Cod_Visibilidad = Visibilidades.Cod_Visibilidad
GO
--------------------------------------------------------------------------------
...
AS
		...
		SET @SQLQuery = 'SELECT * FROM (
		 ...
		 IF (@tipoDoc IS NOT NULL AND @numeroDoc IS NOT NULL)
			 SET @SQLQuery = @SQLQuery + ' AND (Tipo_Doc = @tipoDoc AND Num_Doc = @numeroDoc)'
...
AS BEGIN
	...
	INSERT INTO LOS_INSISTENTES.Compras
	SELECT s.ID_Comprador, p.Cod_Publicacion, NULL, @fechaSistema, s.Monto_Oferta, c.Forma_Pago
	FROM LOS_INSISTENTES.Publicaciones p, 
...
CREATE PROCEDURE LOS_INSISTENTES.sp_AgregarVisibilidad
	(@descripcion nvarchar(255), @costoPublicacion numeric(18,2), @comisionEnvio numeric(18,2),
	 @comisionVenta numeric(18,2), @habilitada bit, @retorno numeric(18,0) output)
AS BEGIN
	INSERT INTO LOS_INSISTENTES.Visibilidades
		(Descripcion, Costo_Publicacion, Comision_Envio, Comision_Venta, Habilitada)
	VALUES
		(@descripcion, @costoPublicacion, @comisionEnvio, @comisionVenta, @habilitada)
...
CREATE PROCEDURE LOS_INSISTENTES.sp_EditarVisibilidad
	(@codVisibilidad numeric(18,0), @descripcion nvarchar(255), @costoPublicacion numeric(18,2),
	 @comisionEnvio numeric(18,2), @comisionVenta numeric(18,2), @habilitada bit)
AS BEGIN
	UPDATE LOS_INSISTENTES.Visibilidades 
...
AS BEGIN
	DELETE FROM LOS_INSISTENTES.Visibilidades WHERE Cod_Visibilidad = @codVisibilidad
END
GO
...
INSERT INTO LOS_INSISTENTES.Funcionalidades (ID_Funcionalidad, Nombre) VALUES (3,'Administrar Rubros');
INSERT INTO LOS_INSISTENTES.Funcionalidades (ID_Funcionalidad, Nombre) VALUES (4,'Administrar Visibilidades');
...
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Administrador', @funcionalidad = 'Administrar Roles';
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Administrador', @funcionalidad = 'Administrar Usuarios';
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Administrador', @funcionalidad = 'Administrar Rubros';
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Administrador', @funcionalidad = 'Administrar Visibilidades';
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Administrador', @funcionalidad = 'Listado Estadístico';				
		
/* Cliente */

EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Cliente', @funcionalidad = 'Comprar / Ofertar';	
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Cliente', @funcionalidad = 'Calificar';
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Cliente', @funcionalidad = 'Historial Cliente';

	
/* Empresas */

EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Empresa', @funcionalidad = 'Administrar Publicaciones';		
EXEC LOS_INSISTENTES.sp_AgregarFuncionalidadRol
	@rol = 'Empresa', @funcionalidad = 'Consultar Facturas';
		

/* Generamos el usuario admin con la password indicada en el enunciado */

SET IDENTITY_INSERT LOS_INSISTENTES.Usuarios ON
INSERT INTO LOS_INSISTENTES.Usuarios(ID_User,Username,Password,Intentos_Login,Habilitado,Publi_Gratuitas) 
	VALUES (0,'admin','E6B87050BFCB8143FCB8DB0170A4DC9ED00D904DDD3E2A4AD1B1E8DC0FDC9BE7',0,1,0);
...
INSERT INTO LOS_INSISTENTES.Visibilidades(Descripcion, Costo_Publicacion, Comision_Venta, Comision_Envio, Habilitada) 
...
UPDATE LOS_INSISTENTES.Visibilidades SET Permite_Envios = 0 WHERE Cod_Visibilidad IN (3,4) -- Bronce y Gratis
...
INSERT INTO LOS_INSISTENTES.Subastas(ID_Comprador, Cod_Publicacion, Fecha_Oferta, Monto_Oferta)
	SELECT ofertador, codpublic, fechaOferta, monto
...
AS BEGIN
	...
	BEGIN
		...
		WHERE Cod_TipoPublicacion = 1 AND Stock = 0)
	END
	...
	DECLARE @codPublicacion numeric(18,0), @visibilidad nvarchar(255), @montoPublicacion numeric(18,2),
			@fechaInicial datetime,	@nroFactura numeric(18,0)

	-- Tengo que generar una factura para cada publicación finalizada, uso un cursor
	DECLARE cursorFinalizadas CURSOR FAST_FORWARD FOR
		SELECT i.Cod_Publicacion, 'Costo de publicación ' + v.Descripcion AS Visibilidad,
		...
		AND d.Cod_EstadoPublicacion != 1

	OPEN cursorFinalizadas 
	FETCH NEXT FROM cursorFinalizadas INTO @codPublicacion, @visibilidad,
	...
	BEGIN 
		...
		FETCH NEXT FROM cursorFinalizadas INTO @codPublicacion, @visibilidad, @montoPublicacion, @fechaInicial
	END
...
AS BEGIN
	...
	SELECT @codPublicacion = i.Cod_Publicacion, @visibilidad = 'Costo de publicación ' + v.Descripcion,
			@montoPublicacion = v.Costo_Publicacion, @fechaInicial = i.Fecha_Inicial
	FROM INSERTED i, LOS_INSISTENTES.Visibilidades v 
	WHERE i.Cod_Visibilidad = v.Cod_Visibilidad AND i.Cod_EstadoPublicacion = @estadoActiva
...
AS BEGIN
	...
	DECLARE @codPublicacion numeric(18,0), @montoCompra numeric(18,2), @idComprador numeric(18,0),
			@fechaOperacion datetime, @permiteEnvio bit;
			
	SELECT  @codPublicacion = Cod_Publicacion, @idComprador = ID_Comprador, @fechaOperacion = Fecha_Operacion,
			@montoCompra = Monto_Compra	FROM inserted
	
	SELECT @permiteEnvio = Permite_Envios FROM LOS_INSISTENTES.Publicaciones WHERE Cod_Publicacion = @codPublicacion
	...
	INSERT INTO LOS_INSISTENTES.Item_Factura 
		...
		'Venta a ' + LOS_INSISTENTES.fc_Nombre(@idComprador) + ' el ' + convert(varchar(25), @fechaOperacion, 103), 
	...
	-- Si permite envíos agrego el concepto del envío

	IF @permiteEnvio = 1
	BEGIN
		INSERT INTO LOS_INSISTENTES.Item_Factura 
		SELECT DISTINCT i.Nro_Factura, p.Cod_Publicacion, 
		'Envío a ' + LOS_INSISTENTES.fc_Nombre(@idComprador) + ' el ' + convert(varchar(25), @fechaOperacion, 103), 
	...
	-- Si es compra inmediata le saco uno de Stock
	UPDATE LOS_INSISTENTES.Publicaciones SET Stock = Stock - 1 
